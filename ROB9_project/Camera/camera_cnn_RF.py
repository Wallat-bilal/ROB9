...
# -------------------------------------------------------------------------
# YOLO model & class config (APPLE-RF)
# -------------------------------------------------------------------------

# Class IDs must match your AppleRF data.yaml:
# 0: apple
# 1: bruise
# 2: pest
# 3: rotten
...
# Which classes count as "defects" for reject_defect decision
DEFECT_CLASSES = {"bruise", "pest", "rotten", "scratch"}

# Minimum red ratio inside a YOLO box for it to be considered "on an apple"
APPLE_MIN_RED_FOR_DEFECT = 0.20  # 20% of pixels in box must be red (tune as needed)
...
def load_yolo_model() -> YOLO:
    ...
    print(f"[YOLO] Loading AppleRF model from: {model_path}")
...
def compute_red_ratio_bgr(image_bgr: np.ndarray) -> float:
    ...
    # Red ranges with reasonably high saturation & value to avoid white/gray
...
def run_yolo_on_frame(
    ...
    # Filter detections: only defect classes with enough redness inside their box
    if r.boxes is not None and len(r.boxes) > 0:
        ...
        for box in r.boxes:
            ...
            # We ignore pure "apple" boxes as defects
            ...
            # Only keep detections that sit on "red-enough" apple region
            if roi_red >= APPLE_MIN_RED_FOR_DEFECT:
                valid_defects.append((cls_name, conf, roi_red))
            else:
    ...
    if decision == "reject_defect":
        ...
        for name, conf, roi_red in valid_defects:
            print(f"  - {name} (conf={conf:.2f}, roi_red={roi_red:.2f})")
    ...
    if num_defects > 0:
        defect_str = ", ".join(
            f"{name}({conf:.2f})" for name, conf, _ in valid_defects
        )
        cv2.putText(
            annotated,
    ...
    try:
        while True:
            ...
            annotated, defects, decision, red_ratio = run_yolo_on_frame(
                ...
                color_image,
                conf_th=0.5,
                red_threshold=0.0,  # e.g. require 50% red for color acceptance
            )
