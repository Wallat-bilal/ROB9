...
# If YOLO confidence is below this, pick it and send to REJECT
YOLO_REJECT_THRESHOLD = 0.70
# Minimum apple confidence to still consider for REJECT (must be <= model(conf=...))
YOLO_LOWCONF_MIN = 0.25

# Redness threshold (your requirement “>= 50% red”)
REDNESS_PICK_THRESHOLD = 0.20
...
# -----------------------------
# Vacuum timing / motion wait tuning
# -----------------------------
# The robot motion is commanded as separate URScript programs. If sleeps are too short,
# your gripper can release "early" simply because the code reached vacuum_off() before
# the robot has finished the move.
ROBOT_MIN_WAIT_S = 0.25        # minimum wait after any movel publish
ROBOT_TIME_PAD_S = 0.35        # extra padding added to estimated travel time

VACUUM_BUILDUP_S = 0.50        # time to wait after vacuum_on() before lifting
VACUUM_RELEASE_DWELL_S = 0.25  # dwell at bin pose before vacuum_off()
...
def _sleep_for_movel(p_from: np.ndarray, p_to: np.ndarray, v: float) -> None:
    """Estimate movel time from XYZ distance / v (TCP speed) + padding, then sleep."""
    try:
        v = float(v)
        dist = float(np.linalg.norm(np.asarray(p_to[:3], dtype=float) - np.asarray(p_from[:3], dtype=float)))
        t = dist / max(v, 1e-3)
    except Exception:
        t = 0.0
    time.sleep(max(ROBOT_MIN_WAIT_S, t + ROBOT_TIME_PAD_S))
...
def do_pick_and_place(
    ...
    _sleep_for_movel(cur, home_pose, MOVE_V_TRAVEL)
    cur = home_pose.copy()

    # above apple
    publish_urscript(urscript_movel(above_pick, a=MOVE_A_TRAVEL, v=MOVE_V_TRAVEL))
    _sleep_for_movel(cur, above_pick, MOVE_V_TRAVEL)
    cur = above_pick.copy()

    # down
    publish_urscript(urscript_movel(down_pick, a=MOVE_A_APPROACH_PICK, v=MOVE_V_APPROACH_PICK))
    _sleep_for_movel(cur, down_pick, MOVE_V_APPROACH_PICK)
    cur = down_pick.copy()

    # vacuum on (can start a bit early, but we MUST keep it ON until we are at the bin)
    if enable_vacuum:
        vacuum_on()
    time.sleep(VACUUM_BUILDUP_S)
    ...
    _sleep_for_movel(cur, lift_pose, MOVE_V_LIFT)
    cur = lift_pose.copy()

    # above bin
    publish_urscript(urscript_movel(above_bin, a=MOVE_A_TRAVEL, v=MOVE_V_TRAVEL))
    _sleep_for_movel(cur, above_bin, MOVE_V_TRAVEL)
    cur = above_bin.copy()

    # down to bin (wait until we're DOWN before releasing)
    publish_urscript(urscript_movel(bin_pose, a=MOVE_A_APPROACH_BIN, v=MOVE_V_APPROACH_BIN))
    _sleep_for_movel(cur, bin_pose, MOVE_V_APPROACH_BIN)
    cur = bin_pose.copy()
    time.sleep(VACUUM_RELEASE_DWELL_S)

    # vacuum off (release at the bin)
    if enable_vacuum:
        vacuum_off()
    time.sleep(0.10)
...
def main():
    ...
    try:
        while True:
            ...
            # If nothing passes CONF_APPLE, still allow a lower-confidence apple and send it to REJECT
            low_conf_reject = False
            if best_apple is None:
                low_candidate = pick_best_apple_detection(results, conf_min=YOLO_LOWCONF_MIN)
                if low_candidate is not None:
                    _xyxy_lc, _conf_lc = low_candidate
                    if _conf_lc < YOLO_REJECT_THRESHOLD:
                        best_apple = low_candidate
                        low_conf_reject = True
            defects = extract_defects(results, defect_names=("pest", "scratch"))

            annotated = results[0].plot()

            low_conf_reject = False  # set True if we accept a <70% apple just to reject it
            ...
            # Decision:
            # - If NOT red enough -> pick it and put it in REJECT
            # - If red enough but has defect -> REJECT
            # - Otherwise -> ACCEPT
            is_reject = (not should_pick) or reject_defect

            # Force REJECT if YOLO confidence was below threshold
            if low_conf_reject:
                is_reject = True

            if is_reject:
                decision = "PICK -> REJECT" if should_pick else "PICK -> REJECT (not red enough)"
            else:
                decision = "PICK -> ACCEPT"

            # If YOLO confidence is low, make the UI explicit (still REJECT)
            if low_conf_reject:
                decision = f"PICK -> REJECT (yolo<{int(YOLO_REJECT_THRESHOLD*100)}%)"
            ...
            if enable_motion and in_bin_zone and (now - last_home_time) > 1.0:
                publish_urscript(urscript_movel(home_pose, a=MOVE_A_TRAVEL, v=MOVE_V_TRAVEL))
                last_home_time = now
