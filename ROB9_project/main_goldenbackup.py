...
# -----------------------------
# VG10 cup XY compensation (end-effector fine tune)
# -----------------------------
# If you blocked all cups except one (e.g. cup at row/col (3,3) in a 4x4),
# enable this to shift the TCP target so the active cup lands on the apple center.
VG10_USE_CUP_XY_COMP = True

# Active cup index in a 4x4 matrix (row, col) using 1..4 indexing.
VG10_ACTIVE_CUP_RC = (3, 3)

# Matrix size (normally 4 x 4 for VG10)
VG10_GRID_ROWS = 4
VG10_GRID_COLS = 4

# Center-to-center spacing between cups (meters). Tune these for your end-effector.
VG10_CUP_PITCH_X = 0.01
VG10_CUP_PITCH_Y = 0.01

# If compensation goes the wrong way, flip the sign(s).
VG10_CUP_X_SIGN = 1.0
VG10_CUP_Y_SIGN = 1.0

# Extra fine trim (meters). Use small values like 0.001..0.005.
VG10_CUP_TRIM_X = 0.0
VG10_CUP_TRIM_Y = 0.0
...
# -----------------------------
# VG10 cup XY compensation helper
# -----------------------------
def vg10_compensate_xy(x: float, y: float) -> Tuple[float, float, float, float]:
    """
    Returns (x_cmd, y_cmd, dx, dy).

    - (x, y) is the apple center in base frame.
    - (dx, dy) is the active-cup offset from the tool center (in base XY meters),
      computed from VG10_ACTIVE_CUP_RC and VG10_CUP_PITCH_X/Y.
    - (x_cmd, y_cmd) is the commanded TCP target so the ACTIVE CUP lands on (x, y).

    If VG10_USE_CUP_XY_COMP is False, returns (x, y, 0, 0).
    """
    if not VG10_USE_CUP_XY_COMP:
        return x, y, 0.0, 0.0

    r, c = VG10_ACTIVE_CUP_RC
    center_r = (VG10_GRID_ROWS + 1) / 2.0
    center_c = (VG10_GRID_COLS + 1) / 2.0

    dx = (c - center_c) * VG10_CUP_PITCH_X * VG10_CUP_X_SIGN + VG10_CUP_TRIM_X
    dy = (r - center_r) * VG10_CUP_PITCH_Y * VG10_CUP_Y_SIGN + VG10_CUP_TRIM_Y

    # Move TCP opposite of cup offset, so the selected cup lands on the apple.
        ...
        while True:
            ...
            if enable_motion and (now - last_pick_time) > PICK_COOLDOWN_S:
                if should_pick and (not in_bin_zone):
                    ...
                    pick_x_cmd, pick_y_cmd, cup_dx, cup_dy = vg10_compensate_xy(bx, by)
                    if VG10_USE_CUP_XY_COMP:
                        print(f"[VG10] cup_rc={VG10_ACTIVE_CUP_RC} dx={cup_dx:.4f} dy={cup_dy:.4f} -> cmd_xy=({pick_x_cmd:.3f},{pick_y_cmd:.3f})")
                    try:
                        do_pick_and_place(
                            cal=cal,
                            home_pose=home_pose,
