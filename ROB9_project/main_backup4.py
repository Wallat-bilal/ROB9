...
# Apple selection
CONF_APPLE = 0.90  # YOLO confidence for "apple" box to be considered
...
# Motion tuning (meters)
PICK_Z_DROP = 0.06       # how far down from workspace Z to touch apple
LIFT_Z_UP = 0.10         # lift after picking
BIN_APPROACH_Z_UP = 0.10
...
def urscript_movel(pose6: np.ndarray, a: float = a, v: float = v) -> str:
...
def do_pick_and_place(
    ...
    safe_z = float(workspace[2])
    pick_z = float(workspace[2] - PICK_Z_DROP)
    ...
    # HOME
    publish_urscript(urscript_movel(home_pose, a=1.2, v=0.25))
    time.sleep(0.6)

    # above apple
    publish_urscript(urscript_movel(above_pick, a=1.2, v=0.25))
    time.sleep(0.6)
...
def main():
    ...
    # ---- Move HOME immediately after camera starts ----
    if enable_motion:
